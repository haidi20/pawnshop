<template>
  ...
  <section v-else class="feature-data-page master-data-page">
    <section class="master-data-page__hero">
      <div class="master-data-page__hero-head">
        ...
        <button type="button" class="btn btn-primary master-data-page__hero-action"
    ...
    <article class="feature-data-page__table-section master-data-page__table-section">
      ...
      <DataTableClientSideComponent :vm="locationTableVm" class="master-data-page__datatable">
        <template #extra-actions>
          <div class="master-data-page__table-tools">
            ...
            <button type="button" class="btn btn-outline-primary master-data-page__table-action"
              @click="vm.openCreateLocationModal()">
              <i class="bi bi-plus-circle" />
              <span>Tambah lokasi</span>
            </button>
            ...
            <td data-label="Aksi">
              <div class="master-data-page__action-cell">
                ...
                <button type="button" class="btn btn-outline-danger btn-sm"
                  :disabled="vm.isDeletingLocation(item.source.id)" @click="confirmDeleteLocation(item)">
        ...
        <template #empty>
          <LocalDbFeedbackStateComponent state="empty" title="Belum ada lokasi penyimpanan"
            description="Tambahkan lokasi pertama agar penyimpanan barang per cabang bisa dikelola."
            note="Pilih cabang yang sesuai lalu simpan lokasi ke database lokal perusahaan aktif." :framed="false"
            compact>
            <template #actions>
              <button type="button" class="btn btn-primary" @click="vm.openCreateLocationModal()">
                Tambah lokasi
              </button>
            </template>
      ...
      <p v-if="locationSearchKeyword" class="master-data-page__search-feedback mb-0">
  ...
  <template v-if="isLocationDialogOpen">
    <section class="modal fade show d-block master-data-page__modal" tabindex="-1" role="dialog" aria-modal="true"
      aria-labelledby="master-location-modal-title" @click.self="closeLocationModal">
        ...
        <div class="modal-content border-0 master-data-page__modal-content">
          <div class="modal-header master-data-page__modal-header">
            <div>
              ...
              <h2 id="master-location-modal-title" class="h5 mb-1">
                {{ locationDialogTitle }}
              </h2>
            ...
            <button type="button" class="btn-close" aria-label="Tutup" :disabled="isSavingLocation"
          ...
          <div class="modal-body master-data-page__modal-body">
            <div class="master-data-page__form-grid">
              <label class="master-data-page__field master-data-page__field--full">
                <span>Cabang</span>
                <select v-model="locationForm.branchId" class="form-select">
                  <option value="">
                    Pilih cabang
                  </option>
              ...
              <label class="master-data-page__field">
                ...
                <input v-model.trim="locationForm.locationCode" type="text" class="form-control"
                  placeholder="Contoh: OFF-MKS">
              </label>

              <label class="master-data-page__field">
                <span>Tipe lokasi</span>
                <input v-model.trim="locationForm.locationType" type="text" class="form-control"
                  placeholder="Contoh: Kantor, Gudang">
              </label>

              <label class="master-data-page__field master-data-page__field--full">
                <span>Nama lokasi</span>
                <input v-model.trim="locationForm.locationName" type="text" class="form-control"
                  placeholder="Nama lokasi penyimpanan">
              </label>

              <label class="master-data-page__field">
                <span>Status</span>
                <select v-model="locationForm.isActive" class="form-select">
                  <option :value="true">
                    Aktif
                  </option>
                  <option :value="false">
          ...
          <div class="modal-footer master-data-page__modal-footer">
            <button type="button" class="btn btn-outline-secondary" :disabled="isSavingLocation"
              @click="closeLocationModal">
              Batal
            </button>
            <button type="button" class="btn btn-primary" :disabled="isSavingLocation" @click="handleSaveLocation">
              <span v-if="isSavingLocation" class="spinner-border spinner-border-sm" aria-hidden="true" />
    ...
    <div class="modal-backdrop fade show master-data-page__modal-backdrop" @click="closeLocationModal" />
  </template>
</template>
