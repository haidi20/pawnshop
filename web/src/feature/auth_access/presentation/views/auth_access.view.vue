<template>
  <section v-if="isLoading" class="card p-4">
    <div class="d-flex align-items-center gap-3">
      ...
      <div>
        <div class="fw-bold">Loading Auth Access</div>
  ...
  <section v-else-if="error" class="card p-4">
    ...
    <button class="btn btn-primary" type="button" @click="vm.getAuthAccessData()">
      Muat ulang
    </button>
  </section>
...
const vm = authAccessViewModel();
const { data, isLoading, error } = storeToRefs(vm);

onMounted(() => {
    void vm.getAuthAccessData();
});
</script>
