<template>
  <LocalDbFeedbackStateComponent v-if="isLoading" state="loading" title="Memuat data support"
  ...
  <LocalDbFeedbackStateComponent v-else-if="error" state="error" title="Gagal memuat data support" :description="error"
    note="Coba muat ulang agar notifikasi dan audit log lokal diproses kembali." action-label="Muat ulang"
    @action="vm.getSupportData()" />

  <LocalDbFeedbackStateComponent v-else-if="data && data.totalRows === 0" state="empty" title="Belum ada data support"
    description="Database lokal perusahaan aktif belum memiliki notifikasi atau audit log."
    note="Riwayat support akan muncul di sini setelah data tercatat." />
