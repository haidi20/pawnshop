<template>
  <template v-if="isOpen">
    <section class="modal fade show d-block pawn-contract-page__filter-modal" tabindex="-1" role="dialog"
      aria-modal="true" aria-labelledby="pawn-contract-filter-modal-title" @click.self="emit('close')">
        ...
        <div class="modal-content border-0 pawn-contract-page__filter-content">
          <div class="modal-header pawn-contract-page__filter-header">
            <div>
              ...
              <h2 id="pawn-contract-filter-modal-title" class="h5 mb-1">
                Filter index gadai
              </h2>
          ...
          <div class="modal-body pawn-contract-page__filter-body">
            <div class="row g-3">
              <div class="col-12">
                <label class="form-label fw-semibold" for="pawn-contract-table-branch-filter">Cabang</label>
                <select id="pawn-contract-table-branch-filter" v-model="draftBranchFilter" class="form-select"
                  :disabled="isBranchLocked">
                  <option v-if="allowAllBranches" value="all">
                    Semua cabang
                  </option>
                ...
                <small v-if="isBranchLocked" class="text-secondary">
                  Cabang user ini dikunci sesuai assignment owner.
                </small>
              </div>

              <div class="col-12">
                <label class="form-label fw-semibold" for="pawn-contract-table-status-filter">Status kontrak</label>
                <select id="pawn-contract-table-status-filter" v-model="draftStatusFilter" class="form-select">
                  <option value="all">
                    Semua status
                  </option>
          ...
          <div class="modal-footer pawn-contract-page__filter-footer">
            ...
            <button class="btn btn-outline-secondary" type="button" :disabled="!hasDraftOrAppliedFilters"
              @click="emit('reset')">
              Reset
            </button>
    ...
    <div class="modal-backdrop fade show pawn-contract-page__filter-backdrop" @click="emit('close')" />
  </template>
</template>
