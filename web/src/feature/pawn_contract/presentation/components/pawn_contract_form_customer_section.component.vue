<template>
  <section id="pawn-contract-form-customer-panel" class="card p-4 tab-pane fade"
    :class="{ 'show active': activeStep === 2 }" role="tabpanel" aria-labelledby="pawn-contract-form-customer-tab">
    ...
    <div class="card border-0 bg-light-subtle mt-3">
      <div class="card-body py-3 px-4">
        <div class="form-check form-switch mb-0">
          <input id="stepTwoAutoInsert" class="form-check-input" type="checkbox" :checked="isStepTwoAutoInsertEnabled"
            @change="handleStepTwoAutoInsertChange">
          <label class="form-check-label fw-semibold" for="stepTwoAutoInsert">
    ...
    <div class="row g-3 mt-1">
      <div class="col-12">
        <label class="form-label" for="customerFullName">Nama lengkap nasabah</label>
        <input id="customerFullName" :value="form.customerFullName" class="form-control"
          :class="{ 'is-invalid': !!fieldErrors.customerFullName }" type="text"
          list="pawn-contract-customer-suggestions" placeholder="Contoh: Andi Saputra" autocomplete="off"
          @input="handleCustomerNameInput" @change="handleCustomerNameInput" @blur="handleCustomerNameInput">
        <datalist id="pawn-contract-customer-suggestions">
          <option v-for="suggestion in customerNameSuggestions" :key="suggestion.key" :value="suggestion.value"
            :label="suggestion.label" />
        </datalist>
        <div class="form-text">
          {{ customerLookupMessage }}
        </div>
        <div v-if="fieldErrors.customerFullName" class="invalid-feedback d-block">
      ...
      <div class="col-12 col-md-6">
        <label class="form-label" for="customerBirthDate">Tanggal lahir</label>
        <input id="customerBirthDate" v-model="customerBirthDateModel" class="form-control" type="date">
      </div>

      <div class="col-12">
        <label class="form-label" for="customerAddress">Alamat lengkap</label>
        <textarea id="customerAddress" v-model="customerAddressModel" class="form-control"
          :class="{ 'is-invalid': !!fieldErrors.customerAddress }" rows="3"
          placeholder="Contoh: Jl. Andi Pangeran Pettarani No. 88, Panakkukang" />
        <div v-if="fieldErrors.customerAddress" class="invalid-feedback d-block">
          {{ fieldErrors.customerAddress }}
        </div>
      </div>

      <div class="col-12 col-md-4">
        <label class="form-label" for="customerCity">Kota / kabupaten</label>
        <input id="customerCity" v-model="customerCityModel" class="form-control"
          :class="{ 'is-invalid': !!fieldErrors.customerCity }" type="text" placeholder="Contoh: Makassar">
        <div v-if="fieldErrors.customerCity" class="invalid-feedback d-block">
          {{ fieldErrors.customerCity }}
        </div>
      </div>

      <div class="col-12 col-md-4">
        <label class="form-label" for="customerPhone">Nomor telepon</label>
        <input id="customerPhone" :value="form.customerPhone" class="form-control"
          :class="{ 'is-invalid': !!fieldErrors.customerPhone }" type="text"
          list="pawn-contract-customer-phone-suggestions" placeholder="Contoh: 081234567890" autocomplete="off"
          @input="handleCustomerPhoneInput" @change="handleCustomerPhoneInput" @blur="handleCustomerPhoneInput">
        <datalist id="pawn-contract-customer-phone-suggestions">
          <option v-for="suggestion in customerPhoneSuggestions" :key="suggestion.key" :value="suggestion.value"
            :label="suggestion.label" />
        </datalist>
        <div class="form-text">
          Ketik nomor telepon untuk menampilkan maksimal 5 nasabah tersedia.
        </div>
        <div v-if="fieldErrors.customerPhone" class="invalid-feedback d-block">
          {{ fieldErrors.customerPhone }}
        </div>
      </div>

      <div class="col-12 col-md-4">
        <label class="form-label" for="customerIdentityType">Jenis identitas</label>
        <select id="customerIdentityType" v-model="customerIdentityTypeModel" class="form-select">
          <option v-for="option in referenceData.identityOptions" :key="option.value" :value="option.value">
            {{ option.label }}
          </option>
        </select>
      </div>
      ...
      <div class="col-12">
        <label class="form-label" for="customerIdentityNumber">Nomor identitas</label>
        <input id="customerIdentityNumber" :value="form.customerIdentityNumber" class="form-control"
          :class="{ 'is-invalid': !!fieldErrors.customerIdentityNumber }" type="text"
          list="pawn-contract-customer-identity-suggestions" placeholder="Contoh: 7371xxxxxxxxxxxx" autocomplete="off"
          @input="handleCustomerIdentityNumberInput" @change="handleCustomerIdentityNumberInput"
          @blur="handleCustomerIdentityNumberInput">
        <datalist id="pawn-contract-customer-identity-suggestions">
          <option v-for="suggestion in customerIdentityNumberSuggestions" :key="suggestion.key"
            :value="suggestion.value" :label="suggestion.label" />
        </datalist>
        <div class="form-text">
          Ketik nomor identitas untuk menampilkan maksimal 5 nasabah tersedia.
        </div>
        <div v-if="fieldErrors.customerIdentityNumber" class="invalid-feedback d-block">
