<template>
  <aside class="admin-sidebar" :class="{ 'is-open': isOpen, 'is-collapsed': isCollapsed }" data-testid="app-sidebar">
    ...
    <nav class="sidebar-nav">
      <template v-for="item in items" :key="item.key">
        <RouterLink v-if="!item.children?.length && item.route" :to="item.route" class="sidebar-link"
          :class="{ active: isItemActive(item.route) }" :data-testid="`sidebar-link-${item.key}`" :title="item.label"
          @click="$emit('navigate')">
        ...
        <section v-else class="sidebar-group"
          ...
          <button class="sidebar-group-head sidebar-group-toggle" type="button" :title="item.label"
            :aria-expanded="isGroupExpanded(item)" @click="toggleGroup(item)">
            <span class="sidebar-link-icon">
              <i class="bi" :class="item.icon" />
            </span>
            <span class="sidebar-link-content">
            ...
            <span class="sidebar-group-chevron" aria-hidden="true">
              <i class="bi" :class="isGroupExpanded(item) ? 'bi-chevron-up' : 'bi-chevron-down'" />
            </span>
          </button>

          <div v-if="isGroupExpanded(item)" class="sidebar-subnav">
            <template v-for="child in item.children" :key="child.key">
              <RouterLink v-if="!child.children?.length" :to="child.route ?? '/dashboard'" class="sidebar-sublink"
                :class="{ active: isItemActive(child.route) }" :data-testid="`sidebar-link-${child.key}`"
                :title="child.label" @click="$emit('navigate')">
                <span class="sidebar-sublink-bullet" />
                <span class="sidebar-sublink-icon">
                  <i class="bi" :class="child.icon" />
                </span>
                <span class="sidebar-sublink-text">{{ child.label }}</span>
              </RouterLink>

              <section v-else class="sidebar-subgroup" :class="{ 'is-active': isNavigationItemActive(child) }"
                :data-testid="`sidebar-group-${child.key}`">
                <component :is="child.route ? RouterLink : 'div'" :to="child.route ?? undefined"
                  class="sidebar-subgroup-link"
                  :class="{ active: child.route ? isItemActive(child.route) : isNavigationItemActive(child) }"
                  :title="child.label" @click="child.route ? $emit('navigate') : undefined">
                  <span class="sidebar-sublink-bullet" />
                  <span class="sidebar-sublink-icon">
                    <i class="bi" :class="child.icon" />
                  </span>
                ...
                <div class="sidebar-subnav sidebar-subnav--nested">
                  <RouterLink v-for="grandChild in child.children" :key="grandChild.key"
                    :to="grandChild.route ?? '/dashboard'" class="sidebar-sublink sidebar-sublink--nested"
                    :class="{ active: isItemActive(grandChild.route) }" :data-testid="`sidebar-link-${grandChild.key}`"
                    :title="grandChild.label" @click="$emit('navigate')">
                    <span class="sidebar-sublink-bullet" />
                    <span class="sidebar-sublink-icon">
                      <i class="bi" :class="grandChild.icon" />
                    </span>
    ...
    <section class="sidebar-danger" data-testid="sidebar-danger-zone">
      <div class="sidebar-danger-badge">
        Danger Zone
      </div>
      <button class="sidebar-danger-button" type="button" :disabled="isResetting" data-testid="sidebar-danger-reset"
        title="Hapus data" @click="resetLocalData">
        <i class="bi" :class="isResetting ? 'bi-arrow-repeat' : 'bi-trash3'" />
...
const resetLocalData = async (): Promise<void> => {
  ...
  const confirmed = window.confirm('Hapus seluruh data lalu logout dari aplikasi?');
  if (!confirmed) {
    return;
  }
  ...
  try {
    ...
    await showSuccessMessage('Data berhasil dihapus', 'Sesi Anda sudah ditutup.', {
      toast: true,
      timer: 1800,
      position: 'top-end'
    });
