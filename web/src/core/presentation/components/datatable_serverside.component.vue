  ...
  <div class="ps-datatable card">
    <div class="ps-datatable__toolbar">
      ...
      <div class="ps-datatable__toolbar-right">
        <form class="ps-datatable__search" @submit.prevent="runSearch">
    ...
    <div class="ps-datatable__surface">
      ...
      <div v-if="isLoading" class="ps-datatable__overlay">
        <div class="ps-datatable__overlay-card">
          ...
          <div class="ps-datatable__overlay-copy">
            <div class="ps-datatable__overlay-title">Memuat data terbaru</div>
            <div>Filter dan halaman tetap dipertahankan agar tidak terasa flicker.</div>
          </div>
        </div>
      </div>
    </div>
...
import { computed, isRef, onBeforeUnmount, useSlots } from 'vue';

import type { DataTableServerSideVM } from '@/core/presentation/view_models/datatable_serverside.vm';
...
const isLoading = computed(() => vmData.value.loading);
...
const emptyStateTitle = computed(() =>
  vmData.value.search ? 'Tidak ada hasil yang cocok' : 'Data masih kosong'
);
const emptyStateNote = computed(() =>
  vmData.value.search
    ? 'Coba gunakan kata kunci lain atau longgarkan filter pencarian.'
    : 'Saat data tersedia, tabel akan tampil dengan format yang sudah konsisten.'
);
const searchPlaceholder = computed(() =>
  normalizedFields.value.length > 0 ? 'Cari data berdasarkan kata kunci...' : 'Cari data...'
);
